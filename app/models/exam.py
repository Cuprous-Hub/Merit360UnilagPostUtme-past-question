...
class Result(db.Model):
    ...
    num_questions_allowed = db.Column(db.Integer, default=30)
...
class TournamentEntry(db.Model):
    """Tracks paid tournament access codes. Each code is generated by admin after payment confirmation."""
    __tablename__ = 'tournament_entries'
    id = db.Column(db.Integer, primary_key=True)
    access_code = db.Column(db.String(10), unique=True, nullable=False, index=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # set when code is used
    is_used = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    used_at = db.Column(db.DateTime, nullable=True)
    # Optional: store payment reference for records
    payment_reference = db.Column(db.String(100), nullable=True)
    payer_name = db.Column(db.String(200), nullable=True)

    def __repr__(self):
        return f'<TournamentEntry code={self.access_code} used={self.is_used}>'
